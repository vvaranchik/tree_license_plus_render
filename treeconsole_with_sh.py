...
def process_license_in_file_(p:subprocess.Popen, path:str, fname, ftype:int, comment_char_start:str, comment_char_end:str='')->None:
...
def try_add_license(fname)->None:
    ...
    if is_licensed(fname, ftype) != True:
        ...
        #if ftype == 0:
        process_license_in_file_(p, fname.path, licensed_filename, ftype, comments[0], comments[1])
        #else:
        #    if ftype == 2:
...
if __name__ == "__main__":
    ...
    sys.stderr = sys.stdout = open(log_path, 'w')
    if license_path == '':
