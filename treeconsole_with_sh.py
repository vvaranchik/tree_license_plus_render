...
def log(logtype:int, str_log:str):
...
def is_all_paths_valid(paths:list[str]):
    for path in paths:
...
def process_license_in_file_(p:subprocess.Popen, path:str, fname, ftype:int, comment_char_start:str='', comment_char_end:str=''):
...
def process_license_in_file(bytestr, path:str, fname:str, ftype:int, comment_char_start='', comment_char_end=''):
    licensed_file = fname
    if os.path.exists(licensed_file):
        #if is_licensed(licensed_file, ftype): #re-check
        log(0, 'License for file "{}" already exists. Skipping...'.format(fname))
        return
    license_filled:list[str]=bytestr.decode('utf-8').split('\r')
    with open(licensed_file, "w") as licensed:
        if ftype != 1:
            first_line = True
            for line in license_filled:
...
def try_add_license(fname):
    ...
    if is_licensed(fname, ftype) != True:
        ...
        #bash_scr = open('./render.sh', 'r').read()
        #log(0, bash_scr)
        cmd = [
            'sh',
            #'./render.sh --filename="{filename}" --root_folder="{root_folder}" --year="{year}" --org_name="{org_name}" {license_pth}'.format(filename = licensed_filename, root_folder = project_path, year = str(), #org_name = "StackSoft", license_pth = license_path)
            './render.sh',
        ...
        #outbytes = subprocess.run(cmd, stdout = subprocess.PIPE, stderr = subprocess.PIPE, shell=True)
        #bb = outbytes.stdout
        #log(0, bb)
        #if outbytes.returncode != 0:
        #    log(1, "error")
        if ftype == 0:
            #licensed.write('/*\n')
            process_license_in_file_(p, fname.path, licensed_filename, ftype, '/*', '*/');
        else:
            if ftype == 2:
                process_license_in_file_(p, fname.path, licensed_filename, ftype, '#');
            else:
...
def process_license(dir_path:str):
    dirs: list = list()
    files: list = list()
...
def process_argument(arg_key:str, args:list[str], it: int):
